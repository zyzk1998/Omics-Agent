...
def list_skills_public(
    ...
    if D > 0:
        if page == 1:
            ...
            offset_db = max(0, (page - 1) * size - D)
            lim_db = size
    else:
        offset_db = (page - 1) * size
        lim_db = size

    page_rows = sorted_rows[offset_db : offset_db + lim_db]
...
try:
    ...
    if agent and tool_retriever:
        try:
            ...
            # 云端多模型：领域智能体上 llm_client 可能为 None，按注册表默认模型创建规划器专用客户端
            if llm_client is None:
                try:
                    llm_client = LLMClientFactory.create_for_model(get_default_chat_model())
                    logger.info("✅ 云端模式：已按 MODEL_ROUTING_TABLE 默认模型创建 WorkflowPlanner 用 LLM 客户端")
                except Exception as _pl_e:
                    logger.warning(
                        "⚠️ 无法为 WorkflowPlanner 创建默认 LLM 客户端，跳过工作流规划器初始化: %s",
                        _pl_e,
                    )
                    llm_client = None
            ...
            if llm_client:
                ...
                logger.warning("⚠️ 无可用 LLM client，跳过工作流规划器初始化")
        except Exception as e:
...
async def plan_workflow(req: WorkflowPlanRequest):
    ...
    try:
        ...
        # SOPPlanner.generate_plan 仅存在于 SOPPlanner；与 chat 侧 WorkflowPlanner.plan 分离
        if not tool_retriever:
            raise HTTPException(status_code=500, detail="工具检索器未初始化")
        from gibh_agent.core.llm_client import LLMClientFactory
        from gibh_agent.core.llm_cloud_providers import get_default_chat_model
        from gibh_agent.core.planner import SOPPlanner

        try:
            _default_model = get_default_chat_model()
        except ValueError as e:
            raise HTTPException(status_code=500, detail=str(e))
        llm_client = LLMClientFactory.create_for_model(_default_model)
