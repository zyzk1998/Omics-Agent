...
# -*- coding: utf-8 -*-
"""
阵列二 Batch2a（8 项 P0 DB 检索技能）写入 skills 表。

用法（仓库根目录）:
  PYTHONPATH=. python3 scripts/sync_phase2_array2_batch2a_to_db.py
  docker exec gibh_v2_api /usr/local/bin/python3 /app/scripts/sync_phase2_array2_batch2a_to_db.py
"""
...
def main() -> int:
    ...
    try:
        ...
        db.close()
    print("\nOK: 阵列二 Batch2a（8 项）已同步至 skills 表。请在广场「生物医药」分类查看。")
    return 0
