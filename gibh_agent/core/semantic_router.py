...
# 运行时请使用 build_semantic_router_system_prompt()，附加动态「工具视野」F 段。
# ---------------------------------------------------------------------------
SEMANTIC_ROUTER_SYSTEM_PROMPT_BASE = """你是 GIBH 智能体的语义路由判定器。后续 user 消息会提供 JSON 上下文（含 query、file_status、mcp_status、session_flags、recent_history）。你只输出一个 JSON 对象：禁止 markdown、禁止代码围栏、禁止任何解释性前缀或后缀、禁止 JSON 内注释或尾逗号。
...
# 兼容旧名：仅静态基座（A–E），不含动态 F；调参文档与 eval 提示中的「SEMANTIC_ROUTER_SYSTEM_PROMPT」指此基座。
SEMANTIC_ROUTER_SYSTEM_PROMPT = SEMANTIC_ROUTER_SYSTEM_PROMPT_BASE


def build_semantic_router_system_prompt() -> str:
    """
    组装完整路由 System Prompt：静态红线 + 动态 SOP/工具清单（Tool-Aware）。
    """
    from gibh_agent.core.capabilities_digest import build_capabilities_digest_for_llm

    digest = build_capabilities_digest_for_llm()
    return (
        SEMANTIC_ROUTER_SYSTEM_PROMPT_BASE
        + "\n\n【F. 系统可用 SOP 与工具视野（动态注入；勿编造清单外能力）】\n"
        + digest
        + "\n\n【F 规则 — task 优先】\n"
        "若用户模糊需求能与上述某一 SOP 域或典型组学分析（如单细胞、转录组、代谢组、空间组学、影像组学、时空动力学等）建立合理对应，"
        "且 file_status 已显示有上传/路径、或用户明确要「跑流程、做分析、出图」：应优先判 **task**，以便系统调用对应 SOP 生成工作流预览卡片；"
        "不要仅因「需求不够具体」就判 clarify。\n"
        "若无法对应任何已列出 SOP、或缺少数据且上下文中无任何组学/文件线索，才判 clarify。\n"
        "\n【输出铁律 — 违反将导致解析失败与系统异常】\n"
        "你必须且只能输出**一个**合法 JSON 对象（仅含 route、confidence、rationale_short 三键），"
        "**绝对不允许**包含 Markdown 代码围栏（如 ``` 或 ```json）、任何前言/后语/空行包裹、"
        "**禁止**输出思考过程、解释文字或「好的，以下是 JSON」等套话；"
        "否则下游解析将失败并可能触发降级与重复澄清。\n"
    )
...
def _extract_json_object(text: str) -> Optional[Dict[str, Any]]:
    """从模型原文中抠出 JSON 对象（Kimi 等可能带 Markdown/废话，见 llm_json_extract）。"""
    return extract_json_object_from_llm_text(text)
...
class SemanticRouter:
    ...
    async def decide_route(self, input_data: RouterInput) -> RouterOutput:
        ...
        messages = [
            {"role": "system", "content": build_semantic_router_system_prompt()},
            {
                "role": "user",
                "content": (
