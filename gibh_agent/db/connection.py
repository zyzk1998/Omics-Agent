...
_sessions_composer_schema_lock = threading.Lock()
...
_messages_index_schema_lock = threading.Lock()
_messages_index_schema_done = False
...
def get_db_session() -> Generator:
    global _users_approval_schema_done, _sessions_composer_schema_done, _sessions_status_schema_done, _feedbacks_status_schema_done, _messages_index_schema_done
    db = SessionLocal()
    try:
        ...
        if not _sessions_composer_schema_done:
            with _sessions_composer_schema_lock:
                if not _sessions_composer_schema_done:
                    from gibh_agent.db.session_composer_schema import (
                        ensure_sessions_composer_draft_column,
                    )

                    ensure_sessions_composer_draft_column(engine)
                    _sessions_composer_schema_done = True
        if not _sessions_status_schema_done:
            with _sessions_status_schema_lock:
                if not _sessions_status_schema_done:
                    from gibh_agent.db.session_status_schema import (
                        ensure_sessions_status_column,
                    )
        ...
        if not _messages_index_schema_done:
            with _messages_index_schema_lock:
                if not _messages_index_schema_done:
                    from gibh_agent.db.message_index_schema import (
                        ensure_messages_session_role_created_index,
                    )

                    ensure_messages_session_role_created_index(engine)
                    _messages_index_schema_done = True
        yield db
    finally:
        db.close()
