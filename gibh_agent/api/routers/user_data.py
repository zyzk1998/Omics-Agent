...
class ClientTraceBody(BaseModel):
    """POST /api/utils/client_trace：桌面/Web 前端上报诊断（写入 api-server 日志，供 monitor-lite Agent Trace 查看）"""
    event: str = "unknown"
    message: str = ""
...
@router.post("/api/utils/client_trace")
def client_trace(
    body: ClientTraceBody = Body(...),
    owner_id: str = Depends(get_current_owner_id),
) -> dict:
    """将前端 LocalGate / Sidecar 失败上下文写入服务端日志（不走浏览器控制台）。"""
    meta_str = ""
    if body.meta is not None:
        try:
            meta_str = json.dumps(body.meta, ensure_ascii=False)[:6000]
        except Exception:
            meta_str = str(body.meta)[:6000]
    detail_s = (body.detail or "")[:8000]
    msg_s = (body.message or "")[:4000]
    log_line = (
        f"[CLIENT_LOCAL_GATE] owner={owner_id} event={body.event} | {msg_s} | detail={detail_s}"
    )
    if meta_str:
        log_line = f"{log_line} | meta={meta_str}"
